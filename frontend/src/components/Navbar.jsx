  ...
  return (
    <div className='flex items-center justify-between font-medium'>
        ...
        <ul className='hidden sm:flex gap-5 text-sm text-gray-700'>
            ...
            <NavLink to = '/collection' className= 'flex flex-col items-center gap-1 display-none'>
                <p>Collection</p>
                <hr className='w-2/4 border-none h-[1.5px] bg-gray-700 hidden'/>
            </NavLink>
        ...
        <div className='flex items-center gap-6'>
            ...
            <div className='group relative'>
                <img onClick={() => token ? null : navigate('/login')} src = {profileIcon} className='w-5 cursor-pointer' alt=''/>
                {/* Drop down menu */}
                {token && 
        ...
        <div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${visible ? "w-full" : "w-0" }`}>
            <div className='flex flex-col text-gray-600'>
                <div onClick={(() => setVisible(false))} className="flex items-center gap-4 p-3 cursor-pointer">
                    <img src={dropdownIcon} className='h-4 rotate-90' alt=''/> 
                    <p>Back</p>
                </div>
